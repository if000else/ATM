...
def repay(card_info):
    ...
    if repay_change > 0:
        functions.colordisplay("You need to repay:%s yuan"%repay_change)
    try:
...
def withdraw(card_info):
    ...
    try:
        change = card_info["change"]
        print("test change:", change)
        amount = input("\033[1;33;1m Input amount:>>\033[0m")
        interest = config.TransType["withdraw"]
        print("test interest:", interest)
...
def transfer(card_info):
    ...
    try:
        ...
        print("current cards: %s"%all_card_info.keys())
...
def calculate(amount,change,type,interest):
    ...
    if type == 'plus':#plus
        ...
        change = change*(1 - interest) - amount
    else:
